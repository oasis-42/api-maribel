...
class Feedback(models.Model):
    ...
    status = models.CharField(
        choices=FeedbackStatus.choices, default=FeedbackStatus.CREATED, max_length=50
    )
...
class SkillFeedback(models.Model):
    skill_feedback_id = models.AutoField(
        primary_key=True, db_column="skill_feedback_id"
    )
    skill = models.ForeignKey(Skill, on_delete=models.DO_NOTHING, db_column="skill_id")
    feedback = models.ForeignKey(
        Feedback, on_delete=models.CASCADE, db_column="feedback_id"
    )
    grade = models.IntegerField()
...
class MotivationalText(models.Model):
    motivational_text_id = models.AutoField(
        primary_key=True, db_column="motivational_text_id"
    )
