...
from core.models import RefinedEssayText, OriginalEssayText, Theme, UserConfig, MotivationalText
from core.serializers import RefinedEssayTextSerializer, OriginalEssayTextSerializer, ThemeSerializer, UserConfigSerializer, MotivationalTextSerializer, CapturedPictureSerializer, FeedbackDtoSerializer, UserSerializer
...
from core.views.authentication import password_reset_confirm, activate, RegisterView as UserRegisterView

from drf_spectacular.views import SpectacularAPIView, SpectacularSwaggerView

urlpatterns = [
    path('api/feedbacks/refined-essay/<int:pk>', RefinedEssayView.as_view(), name='essay_corrected_text_retrieve'),
    path('api/feedbacks/original-essay/<int:pk>', OriginalEssayView.as_view(), name='essay_original_text_retrieve'),
    path('api/configs/<int:pk>', UserConfigDetailView.as_view(), name='user_config_update_retrieve'),
    path('api/themes', ThemeListView.as_view(), name='themes_list'),
    path('api/text-extraction', TextExtractionProcessingView.as_view(), name='text_extraction_process'),
    path('api/ocr/base64', TextExtractionProcessingView.as_view(), name='text_extraction_process_ocr'), # deprecated
    path('api/ocr/feedback', FeedbackProcessingView.as_view(), name='feedback_process_ocr'), # deprecated
    path('api/feedbacks', FeedbackProcessingView.as_view(), name='feedback_process'),
    path('api/schema/', SpectacularAPIView.as_view(), name='schema'),
    path('api/schema/swagger-ui/', SpectacularSwaggerView.as_view(url_name='schema'), name='swagger-ui'),
    path('api/auth/', include('djoser.urls')),
    path('api/auth/', include('djoser.urls.jwt')),
    path('api/auth/register/', UserRegisterView.as_view(), name='register'), # Adicionado endpoint de registro
    ...
    path('api/motivational-texts/theme/<int:theme_id>/', MotivationalTextView.as_view(), name='motivational-texts-by-theme'),
]
