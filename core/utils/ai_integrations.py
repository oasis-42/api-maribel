...
from ai_prompts import EssayAnalysisPrompt, OcrCorrectionPrompt, RefinedEssayPrompt
...
def _get_corrected_text(ocr_response, extracted_text):
    ...
    try:
        ...
        print(
            f"Erro ao processar resposta do ChatGPT (correção OCR): {e}, Content: {chat_gpt_response.choices[0].message.content if chat_gpt_response.choices else 'No content'}"
        )
...
def _get_average_confidence(ocr_response):
    ...
    return (
        sum(confidences_per_block) / len(confidences_per_block)
        if confidences_per_block
        else 0
    )
...
def analyse_essay_with_gpt(text, theme):
    ...
    return _send_to_chat_gpt(
        prompt_content, filter_by_theme, text, "análise de redação"
    )
...
def refine_essay_with_gpt(text, theme):
    ...
    return _send_to_chat_gpt(
        prompt_content, filter_by_theme, text, "refinamento de redação"
    )
...
def _send_to_chat_gpt(prompt_content, filter_by_theme, text, context_name=""):
    ...
    try:
        ...
        print(
            f"Erro ao processar resposta do ChatGPT ({context_name}): {e}, Content: {chat_gpt_response.choices[0].message.content if chat_gpt_response.choices else 'No content'}"
        )
        return {}
